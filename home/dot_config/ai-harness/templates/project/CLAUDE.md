...
- Use Codex or another coding worker only for a scoped vertical slice with clear files and
  verification. Inspect worker output for acceptance and code quality; run `implementation-review`
  when the result is substantial, risky, hard to inspect, or required by the plan.
...
- Ask for Codex second review on risky specs, broad implementation plans, large diffs,
  security-sensitive work, or stuck debugging when available.
