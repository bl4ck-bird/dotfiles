...
sessions and project work do not lose them. Implements the harness "Retro" phase and bridges the
Memory Candidates pattern with the host agent's memory system.
...
- A review skill (`spec-compliance-review`, `code-quality-review`, `security-review`,
  `second-review`, `pressure-test`) reported "Memory candidates" in its output.
...
- **Global** (host agent's memory directory, e.g. `~/.claude/projects/<project-slug>/memory/`):
...
- **Project-local** (`.ai-harness/reviews/YYYY-MM-DD-retro-<topic>.md` or `.ai-harness/DECISIONS/`):
  - Specific to this codebase, team, or product context.
  - Useful when picking up the project after a long pause.
  - May be referenced from `.ai-harness/CURRENT.md`.
- **Neither (skip)**:
...
Match the host agent's memory format. Example for Claude global memory (one file per fact, plus a
one-line pointer in the memory index `MEMORY.md`):
