...
- There is a current acceptance artifact with acceptance criteria: spec, PRD, issue, review finding, or approved task.
- There is an implementation plan with file responsibility mapping.
- The plan passed primary review through `plan-review`, or accepted risk is recorded.
- Core docs were read: `AGENTS.md`, `CONTEXT.md`, `docs/CURRENT.md`, `docs/AGENT_WORKFLOW.md`, the current acceptance artifact/plan, and relevant code.
- Conditional docs were read only when relevant: `CONTEXT-MAP.md`, `docs/ARCHITECTURE.md`, `docs/DOMAIN_MODEL.md`, `docs/DATA_MODEL.md`, `docs/SECURITY_MODEL.md`, `docs/TESTING_STRATEGY.md`, and durable decisions.
...
Use `bounded-loop` instead of this skill when the user asks the agent to continue autonomously toward a goal across repeated iterations. Do not run open-ended loops from `execute-plan`.
...
6. Run `implementation-review` after substantial slices, plus `architecture-review`, `security-review`, or `docs-review` when the slice touches those concerns.
...
- Give the worker the exact acceptance artifact, plan task, allowed files or modules, relevant docs to read, constraints, verification commands, and return format.
...
- Acceptance compliance: does the result match the accepted artifact, plan task, and acceptance criteria?
- Code quality: are tests, maintainability, DDD/SOLID fit, file responsibility, and safety acceptable?

Use separate review passes, `implementation-review`, focused reviewer subagents, or `second-review` only when the worker output is substantial, risky, hard to inspect, or required by the plan. Fix or explicitly accept findings before marking the task complete.
...
For exploratory repo analysis, prefer a batch of two or more independent concerns when practical. A single focused reviewer subagent is acceptable for architecture, test, docs, security, or requested `second-review` checkpoints.
...
- Second review: independent Codex review when required or useful for risky specs, plans, large diffs, architecture changes, weak tests, or security-sensitive work.
